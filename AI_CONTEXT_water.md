# 🍄 CARTOMORILLES — AI Context v2.3.5 | 2026-03-26 17:19 UTC | full | session:water

> Auto-généré par `prepare_context.py` — optimisé Claude Opus
> Project hash: `c09288dc00`

> 🔀 SESSION PARALLÈLE : **water**
> Branche git : `session/water`
> Créée le : 2026-03-26T17:19:08.051070+00:00
>
> **Fichiers modifiables (verrouillés) :**
> - 🔒 `landcover_detector.py` (exclusif)
>
...
- SESSION 'water' : modifier UNIQUEMENT landcover_detector.py
</rules>
...
- Ne PAS modifier : config.py, data_loader.py, grid_builder.py, scoring.py, visualize.py, species_enricher.py, main.py, weather.py
</forbidden>
...
- np.asarray() en entrée des fonctions de score (79× trouvé)
- np.full_like + masque valid pour NaN-safety (2× trouvé)
- np.clip en sortie des scores [0, 1] (38× trouvé)
- Logger avec messages descriptifs pour chaque étape (254× trouvé)
...
├── config.py                  ⏭️ (715L) — 🍄 CARTOMORILLES — Configuration du modèle (v2.2.0) (hors scope)
...
├── visualize.py               ⏭️ (1250L) — visualize.py — Cartomorilles v2.3.5 (hors scope)
├── landcover_detector.py      ✅ (1153L) — landcover_detector.py — Détection landcover par... ◄ SESSION
...
### config.py ⏭️ (hors scope cette session)
...
**Code** : 10 modules | 14,627L total | 11,664L code | 44 fn | 35 cls
**Data** : 48 fichiers | 413.4 MB
**Santé** : 3 TODOs | 10 décisions verrouillées

<current_focus>
FOCUS DE CETTE SESSION : water — landcover_detector.py
DESCRIPTION : water detector
HORS SCOPE (ne pas toucher) : config.py, data_loader.py, grid_builder.py, main.py, scoring.py, species_enricher.py, visualize.py, weather.py
...
INTERDIT: ___
SESSION: water
BRANCHE: session/water
</checkpoint>

---
_Généré le 2026-03-26 17:19 UTC | 579 lignes | ~1174 lignes restantes pour prompt + code_